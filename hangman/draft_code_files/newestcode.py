...
class hangmanGUI:
    def __init__(self):
        ...
        self.sports_dict = {"volleyball": ["Sport with a net and a ball", "Bump, set, spike!", "Beach or indoor activity",
                                      "Six players on each side"],
                       "soccer": ["Global ball game", "Goalposts and a net", "No hands allowed!",
                                  "Played with eleven players on each side"],
                       "basketball": ["Dribbling and shooting", "Slam dunks and layups", "Hoops and a round ball",
                                      "Played with 5 players on each side"]}
        self.sports_list = list(self.sports_dict.keys())

        self.library_dict = {"research": ["You need to understand citations", "Usually starts with a hypothesis",
                                     "Evaluates the existing literature"],
                        "archives": ["Access to rare materials", "Preservation and conservation",
                                     "Macalester History kept here"],
                        "worldcat": ["The default database Macalester students use",
                                     "Helps find items in other libraries as well",
                                     "This database isn't limited to storing only books"]}
        ...
        self.inputted_letters_label = tk.Label(self.mainWin,
                                               font="Arial 16",
                                               relief=tk.RIDGE,
                                               text=("You have already inputted the following letters: " + str(
                                                   self.guessed_letters)))
