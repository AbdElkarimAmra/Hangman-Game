...
========================================================================================================================
========================================================================================================================
The code starts with import statements that are all needed to execute the program.
The code has one class - the hangmanGUI class.
The script code at the very end simply runs the class.
The hangmanGUI class has 1 constructor function and 28 other functions needed to run the constructor.
The constructor function also contains the three dictionaries used in this code - the Macalester sports, the
    Macalester library and the Digital Resources Center dictionaries. The words that shall be guessed by
    players are the keys in the dictionaries - and their hints are the values of the keys as lists of strings.
The constructor function also contains the initializing accumulator variables that are used later in the functions.
When the main window is created in the constructor - the hangman images are stored. In total there are three lists of
    images - each for use in varying difficulty games.

Enjoy the code and the game. Thank you!
========================================================================================================================
"""
...
class hangmanGUI:
    def __init__(self):
        ...
        self.mainWin.config(bg="bisque")


        # THE HANGMAN IMAGES-------------------------------------------------
        macalesterlogo = Image.open("paintings/Macalester_College_Logo.png")
        self.macLogo=ImageTk.PhotoImage(macalesterlogo)
        # Images for the easy level
        ...
        # Combining the easy level images in a list
        ...
        # Combining the medium level images in a list
        ...
        # Combining the hard level images in a list
        ...
        # Continuation of constructing the main game window---------------
        ...
        self.hangedGuyLabel = tk.Label(self.mainWin,
                                       image=self.easy_hangman_images_list[0],
                                       relief=tk.GROOVE,
                                       bd=10)
        ...
        # Label for showing information about the level of difficulty currently being played
        ...
        # Label for addressing invalid inputs
        ...
        # Label for showing the result of the game
        self.result_label = tk.Label(self.mainWin, bg="bisque")
        self.result_label.grid(row=8, column=0, columnspan=3)

        # Label for showing the requested hint
        self.hint_label = tk.Label(self.mainWin,
                                   font="Arial 25",
                                   bg="bisque")
        self.hint_label.grid(row=9, column=0, columnspan=3)

        # Label for showing the word or the word blanks
        ...
        # CODE FOR THE SPLASH SCREEN OF THE MAIN MENU-------------------------------------------------------------------
        # --------------------------------------------------------------------------------------------------------------
        # Withdrawing the main window to deiconify the splash screen of the menu.
        ...
        self.splashScreen.config(bg="bisque")
        ...
        #Macalester's logo at the bottom
        maclogolabel=tk.Label(self.splashScreen,
                              image=self.macLogo,
                              bg="bisque")
        maclogolabel.grid(row=5, column=0, columnspan=2, pady=20)
        #---------------------------------------------------------------------------------------------------------------
        ...
        self.instructions_screen.config(bg="bisque")
        ...
        self.easy_level_screen.config(bg="bisque")
        ...
        self.medium_level_screen.config(bg="bisque")
        ...
        self.hard_level_screen.config(bg="bisque")
    ...
    #DEFINING FUNCTION FOR CLICKING INSTRUCTIONS AND CLICKING BACK
    def clicked_instructions(self):
        """
        the "clicked_instructions" function simply deiconifies the instructions screen
        """
        self.instructions_screen.deiconify()
    def clicked_back(self):
        """
        the "clicked_back" function withdraws the instructions screen.
        """
        self.instructions_screen.withdraw()
    ...
    def clicked_easy(self):
        """
        This function deals with multiple variables, sets values to some accumulators and takes actions when the user
        clicks the easy button in the main menu.
        """
    ...
    def clicked_medium(self):
        """
        This function deals with multiple variables, sets values to some accumulators and takes actions when the user
        clicks the medium button in the main menu.
        """
    ...
    def clicked_hard(self):
        """
        This function deals with multiple variables, sets values to some accumulators and takes actions when the user
        clicks the hard button in the main menu.
        """
    ...
    def easy_clicked_ok(self):
        """This function takes out actions in the case of the user clicking OK in the "EASY" window. This function
        also creates a new restart button in the main menu called "Change settings" after having configured settings."""
        if "hm" not in self.all_dictionaries:
            ...
            self.easy_level_screen.withdraw()
            self.startButton.config(bg="pale green", fg="DarkGreen")
            self.choosing_word()
            # Creating a RESTART button on the main menu again
    ...
    def medium_clicked_ok(self):
        """This function takes out actions in the case of the user clicking OK in the "MEDIUM" window. This function
        also creates a new restart button in the main menu called "Change settings" after having configured settings."""
        if "hm" not in self.all_dictionaries:
            self.choose_a_level.config(text="Now start the game!")
            self.easy_button.config(state="disabled")
    ...
    def hard_clicked_ok(self):
        """This function takes out actions in the case of the user clicking OK in the "HARD" window. This function
        also creates a new restart button in the main menu called "Change settings" after having configured settings."""
        if "hm" not in self.all_dictionaries:
            self.choose_a_level.config(text="Now start the game!")
            self.easy_button.config(state="disabled")
    ...
    def restartResponse(self):
        """Takes no inputs and restarts everything in the game, puts all the variables, images, labels in their initial
        stages."""
        # #Updating the hangman picture
        if self.difficulty_level == "Easy":
            self.hangedGuyLabel.config(image=self.easy_hangman_images_list[0])
        elif self.difficulty_level == "Medium":
            self.hangedGuyLabel.config(image=self.medium_hangman_images_list[0])
        elif self.difficulty_level == "Hard":
            self.hangedGuyLabel.config(image=self.hard_hangman_images_list[0])
        ...
        self.startButton.config(bg="tomato", fg="maroon")
        self.easy_ok_button.config(bg="tomato", fg="maroon")
        self.medium_ok_button.config(bg="tomato", fg="maroon")
        self.hard_ok_button.config(bg="tomato", fg="maroon")
    ...
    def easy_clicked_sports(self):
        """
        Doesn't take any inputs. By using a number of conditionals, this function checks if the Macalester sports are
        already in the list of the dictionaries that the user has chosen - and thus takes actions based on that.
        """
        if "hm" in self.all_dictionaries:
        ...
        if "Macalester Sports" in self.easy_list:
            ...
            self.easy_description.config(text="You have chosen the following dictionaries:\n" + self.cleanString(str(self.easy_list)))
            self.easy_ok_button.config(bg="pale green", fg="DarkGreen")
            self.desc_label.config(text="The word could be related to one of the following topics:\n" + self.cleanString(str(self.easy_list)))
    ...
    def easy_clicked_library(self):
        """
        Doesn't take any inputs. By using a number of conditionals, this function checks if the Macalester library are
        already in the list of the dictionaries that the user has chosen - and thus takes actions based on that.
        """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
    ...
    def easy_clicked_drc(self):
        """
        Doesn't take any inputs. By using a number of conditionals, this function checks if the Macalester DRC are
        already in the list of the dictionaries that the user has chosen - and thus takes actions based on that.
        """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
    ...
    def medium_clicked_sports(self):
        """Same as the easy_clicked_sports function, but for the "MEDIUM" screen. """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
        if "Macalester Sports" in self.medium_list:
            ...
            self.medium_description.config(text="You have chosen the following dictionaries:\n" + self.cleanString(str(self.medium_list)))
            self.medium_ok_button.config(bg="pale green", fg="DarkGreen")
            self.desc_label.config(text="The word could be related to one of the following topics:\n" + self.cleanString(str(self.medium_list)))
            if self.sports_list not in self.all_dictionaries:
                self.all_dictionaries.append(self.sports_list)
                #print(self.all_dictionaries)
    ...
    def medium_clicked_library(self):
        """Same as the easy_clicked_library function, but for the "MEDIUM" screen. """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
    ...
    def medium_clicked_drc(self):
        """Same as the easy_clicked_drc function, but for the "MEDIUM" screen. """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
    ...
    def hard_clicked_sports(self):
        """Same as the easy_clicked_sports function, but for the "HARD" screen. """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
        if "Macalester Sports" in self.hard_list:
            ...
            self.hard_description.config(text="You have chosen the following dictionaries:\n" + self.cleanString(str(self.hard_list)))
            self.hard_ok_button.config(bg="pale green", fg="DarkGreen")
            self.desc_label.config(text="The word could be related to one of the following topics:\n" + self.cleanString(str(self.hard_list)))
            if self.sports_list not in self.all_dictionaries:
                self.all_dictionaries.append(self.sports_list)
                #print(self.all_dictionaries)
    ...
    def hard_clicked_library(self):
        """Same as the easy_clicked_library function, but for the "HARD" screen. """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
    ...
    def hard_clicked_drc(self):
        """Same as the easy_clicked_drc function, but for the "HARD" screen. """
        if "hm" in self.all_dictionaries:
            self.all_dictionaries.remove("hm")
    ...
    def choosing_word(self):
        """The "choosing_word" function takes no inputs, but makes use of existing variables, as well as the random
        module to randomly choose a word from the dictionaries that the user has chosen. It returns the chosen word."""
    ...
    def updating_hangman(self, mistakes):
        """Takes mistakes as an input - it looks at how many mistakes there are - and updates the hanged guy image
        list based on which difficulty level has been chosen."""
        if self.difficulty_level=="Easy":
    ...
    def validInput(self):
        """By using a number of conditionals - tests whether the user's input is valid and returns False in a "valllid"
        Boolean variable if one of the rules is broken by the user."""
    ...
    def checking(self):
        """The "checking" function is binded to the "GUESS" button. It checks whether the user's input (which must
        be asserted that it's valid) is in the word or not, and then takes actions based on the result. It also
        considers the possibility of the game ending and calls the appropriate function for that. """
    ...
    def ending(self, result):
        """Has a conditional that considers whether the game was won or not (based on what happened in the "checking"
        function, and takes actions based on that."""
        if result == "Win":
    ...
    def hinting(self):
        """It's binded to the "HINT" button and considers which difficulty level  has been chosen by the user - which
        dictionaries are at play, then randomly chooses a hint from the dictionary key of the randomly chosen word.
        Any used hints are removed from the key-list, so that if the user clicks "HINT" again, they won't get the same
        hint more than once."""
        if self.difficulty_level=="Easy":
            if self.num_hints>=3:
                self.hint.config(state="disabled")
    ...
    def cleanString(self, inpStr):
        """
        A recursive function that takes a string list as an input and clears all the punctuation but commas.
        """
    ...
    def go(self):
        """Runs the whole class whenever the method .go() is called"""
        self.mainWin.mainloop()

#
# ============================================= END OF THE HANGMANGUI CLASS ============================================
